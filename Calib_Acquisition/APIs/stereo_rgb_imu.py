...
try:
    ...
    path1 = os.path.join(parent_dir, "SVI_Calib_i_%s" % (m.name))
    os.mkdir(path1)
...
def create_main_actors(start_pose, waypoint):
    ...
    bp_rgb.set_attribute('sensor_tick', '0.005')
    ...
    camera_rgbLL = world.spawn_actor(
        bp_rgb,
    ...
    return vehicle, camera_rgbL, camera_rgbLL, camera_rgbR, Gnss_sensor, Imu_sensor
...
def _RGBL_sensor_callback(sensor_data, sensor_name, display):
    ...
    ts = sensor_data.timestamp*(10**9)
    if self.recording:
        sensor_data.save_to_disk('SVI_Calib_i_%s/rgbL/frames/%d.png' % (m.name, ts))
        file_rgbL.write("%d,%d.png \n" % (ts, ts))
        
def _RGBLL_sensor_callback(sensor_data, sensor_name, display):
    return
...
def _RGBR_sensor_callback(sensor_data, sensor_name, display):
    ...
    ts = sensor_data.timestamp*(10**9)
    if self.recording:
        sensor_data.save_to_disk('SVI_Calib_i_%s/rgbR/frames/%d.png' % (m.name, ts))
...
def main():
    ...
    try:
        ...
        vehicle, camera_rgbL, camera_rgbLL, camera_rgbR, Gnss_sensor, Imu_sensor = create_main_actors(
            start_pose, waypoint)
        file_veh_sim = open(os.path.join(path11, '%s_simulation.txt' %
                                         get_actor_display_name(vehicle, truncate=200)), "a")

        camera_rgbLL.listen(lambda data: _RGBLL_sensor_callback(
            data, "camera_rgbLL", display))
        sensor_list.append(camera_rgbLL)

...
try:
    ...
    path1 = os.path.join(parent_dir, "ESVI_Calib_i_%s" % (m.name))
    os.mkdir(path1)
...
def _DVSR_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        cv2.imwrite('ESVI_Calib_i_%s/davisR/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)), dvs_img)
        file_davisR.write("%d,%d,%d.png \n" % (dvs_events[0]['t'], dvs_events[-1]['t'], sensor_data.timestamp*(10**9)))
        np.savez_compressed('ESVI_Calib_i_%s/davisR/events/%d' % (m.name, sensor_data.timestamp*(10**9)), t=dvs_events[:]['t'], x=dvs_events[:]['x'], y=dvs_events[:]['y'], p=dvs_events[:]['pol'])
        
def _DVSL_sensor_callback(sensor_data, sensor_name, display):
    dvs_img, dvs_events = draw_dvsL_image(display, sensor_data)
    if self.recording:
        cv2.imwrite('ESVI_Calib_i_%s/davisL/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)), dvs_img)
        file_davisL.write("%d,%d,%d.png \n" % (dvs_events[0]['t'], dvs_events[-1]['t'], sensor_data.timestamp*(10**9)))
        np.savez_compressed('ESVI_Calib_i_%s/davisL/events/%d' % (m.name, sensor_data.timestamp*(10**9)), t=dvs_events[:]['t'], x=dvs_events[:]['x'], y=dvs_events[:]['y'], p=dvs_events[:]['pol'])

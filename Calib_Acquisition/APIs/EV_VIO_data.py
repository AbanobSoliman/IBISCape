...
try:
    ...
    path1 = os.path.join(parent_dir, "Full_Calib_stereo_i_%s" % (m.name))
    os.mkdir(path1)
    path2 = os.path.join(path1, "davisL")
    os.mkdir(path2)
    path6 = os.path.join(path2, "frames")
    os.mkdir(path6)
    path17 = os.path.join(path2, "events")
    os.mkdir(path17)
    path13 = os.path.join(path1, "davisR")
    os.mkdir(path13)
    path14 = os.path.join(path13, "frames")
    os.mkdir(path14)
    path16 = os.path.join(path13, "events")
    os.mkdir(path16)
    ...
    print("Creation of the directory %s failed" % path16)
    print("Creation of the directory %s failed" % path17)
else:
    ...
    print("Successfully created the directory %s " % path16)
    print("Successfully created the directory %s " % path17)
...
def draw_dvsR_image(surface, image_dvs):
    ...
    surface.blit(image_surface, (480, 360))
...
def draw_dvsL_image(surface, image_dvs):
    ...
    image_surface = pygame.surfarray.make_surface(dvs_img.swapaxes(0, 1))
    # image_surface.set_alpha(100)
    surface.blit(image_surface, (0, 480))
...
def draw_rgbR_image(surface, image_rgbR):
    ...
    image_surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    # image_surface.set_alpha(100)
    surface.blit(image_surface, (360, 0))
...
def create_main_actors(start_pose, waypoint):
    ...
    bp_rgb.set_attribute('image_size_x', '480')
    bp_rgb.set_attribute('image_size_y', '360')
    ...
    # Modify the basic attributes of the blueprint
    bp_semseg.set_attribute('image_size_x', '480')
    bp_semseg.set_attribute('image_size_y', '360')
    bp_semseg.set_attribute('fov', '90.0')
    ...
    # Modify the basic attributes of the blueprint
    bp_depth.set_attribute('image_size_x', '480')
    bp_depth.set_attribute('image_size_y', '360')
    bp_depth.set_attribute('fov', '90.0')
    ...
    # Modify the basic attributes of the blueprint
    bp_dvs.set_attribute('image_size_x', '480')
    bp_dvs.set_attribute('image_size_y', '360')
    bp_dvs.set_attribute('fov', '90.0')
    bp_dvs.set_attribute('sensor_tick', '0.001')
    # Modify the lens distortion attributes
    ...
    bp_dvs.set_attribute('positive_threshold', '0.42')
    bp_dvs.set_attribute('negative_threshold', '0.42')
    ...
    Imu_sensor = world.spawn_actor(
        bp, carla.Transform(carla.Location(x=0.0, y=0.0, z=0.0), carla.Rotation(pitch=0.0, roll=0.0, yaw=0.0)), attach_to=vehicle, attachment_type=carla.AttachmentType.Rigid)

    return vehicle, camera_rgbL, camera_rgbR, camera_semseg, camera_depth, camera_dvsL, camera_dvsR, Gnss_sensor, Imu_sensor
...
def _RGBL_sensor_callback(sensor_data, sensor_name, display):
    draw_rgbL_image(display, sensor_data)
    if self.recording:
        sensor_data.save_to_disk('Full_Calib_stereo_i_%s/rgbL/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
        file_rgbL.write("%d,%d.png \n" % (sensor_data.timestamp*(10**9), sensor_data.timestamp*(10**9)))
        
def _RGBR_sensor_callback(sensor_data, sensor_name, display):
    draw_rgbR_image(display, sensor_data)
    if self.recording:
        sensor_data.save_to_disk('Full_Calib_stereo_i_%s/rgbR/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
...
def _DVSR_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        cv2.imwrite('Full_Calib_stereo_i_%s/davisR/frames/%d.png' % (m.name, dvs_events[0]['t']), dvs_img)
        file_davisR.write("%d,%d,%d.png \n" % (dvs_events[0]['t'], dvs_events[-1]['t'], sensor_data.timestamp*(10**9)))
        np.savez_compressed('Full_Calib_stereo_i_%s/davisR/events/%d' % (m.name, sensor_data.timestamp*(10**9)), t=dvs_events[:]['t'], x=dvs_events[:]['x'], y=dvs_events[:]['y'], p=dvs_events[:]['pol'])
        
def _DVSL_sensor_callback(sensor_data, sensor_name, display):
    dvs_img, dvs_events = draw_dvsL_image(display, sensor_data)
    if self.recording:
        cv2.imwrite('Full_Calib_stereo_i_%s/davisL/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)), dvs_img)
        file_davisL.write("%d,%d,%d.png \n" % (dvs_events[0]['t'], dvs_events[-1]['t'], sensor_data.timestamp*(10**9)))
        np.savez_compressed('Full_Calib_stereo_i_%s/davisL/events/%d' % (m.name, sensor_data.timestamp*(10**9)), t=dvs_events[:]['t'], x=dvs_events[:]['x'], y=dvs_events[:]['y'], p=dvs_events[:]['pol'])
    
def _DEPTH_sensor_callback(sensor_data, sensor_name, display):
    sensor_data.convert(cc.LogarithmicDepth)
    if self.recording:
        sensor_data.save_to_disk('Full_Calib_stereo_i_%s/depth/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
...
def _SSEG_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('Full_Calib_stereo_i_%s/semantic/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
...
def main():
    ...
    try:
        ...
        vehicle, camera_rgbL, camera_rgbR, camera_semseg, camera_depth, camera_dvsL, camera_dvsR, Gnss_sensor, Imu_sensor = create_main_actors(
            start_pose, waypoint)
        file_veh_sim = open(os.path.join(path11, '%s_simulation.txt' %
                                         get_actor_display_name(vehicle, truncate=200)), "a")

        camera_rgbL.listen(lambda data: _RGBL_sensor_callback(
            data, "camera_rgbL", display))
        sensor_list.append(camera_rgbL)
        camera_rgbR.listen(lambda data: _RGBR_sensor_callback(
            data, "camera_rgbR", display))
        sensor_list.append(camera_rgbR)

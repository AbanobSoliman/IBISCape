    ...
    def restart(self):
        ...
        while self.player is None:
            if not self.map.get_spawn_points():
                ...
                sys.exit(1)
            spawn_Point = self.map.get_spawn_points()
            spawn_point = spawn_Point[32]
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
        # Set up the sensors.
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording and self.sensors[self.index][0].startswith('sensor.camera.dvs'):
            outputfile = 'DVS_frame/%s.png' % (dvs_events[0]['t'])
            cv2.imwrite(outputfile,dvs_img)
        elif self.recording and self.sensors[self.index][0].startswith('sensor.camera.rgb'):
            image.save_to_disk('RGB_frame/%08d' % image.frame)
        elif self.recording and self.sensors[self.index][0].startswith('sensor.camera.depth'):
            image.save_to_disk('D_frame/%08d' % image.frame)
        elif self.recording and self.sensors[self.index][0].startswith('sensor.camera.semantic_segmentation'):
            image.save_to_disk('SSeg_frame/%08d' % image.frame)

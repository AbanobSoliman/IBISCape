...
try:
    ...
    path1 = os.path.join(parent_dir, "Sync_Calib_stereo_i_%s" % (m.name))
    os.mkdir(path1)
...
def draw_image(surface, image_dvs, image_rgbL, image_rgbR, image_semseg, image_depth, t_world):
    ...
    if self.recording:
        outputfile = 'Sync_Calib_stereo_i_%s/davis/frames/%d.png' % (m.name, t_world)
        cv2.imwrite(outputfile, dvs_img)
        image_rgbL.save_to_disk('Sync_Calib_stereo_i_%s/rgbL/frames/%d.png' % (m.name, t_world))
        image_rgbR.save_to_disk('Sync_Calib_stereo_i_%s/rgbR/frames/%d.png' % (m.name, t_world))
        image_depth.save_to_disk('Sync_Calib_stereo_i_%s/depth/frames/%d.png' % (m.name,t_world))
        image_semseg.save_to_disk('Sync_Calib_stereo_i_%s/semantic/frames/%d.png' % (m.name, t_world))
...
def create_main_actors(start_pose, waypoint):
    ...
    return vehicle, camera_rgbL, camera_rgbR, camera_semseg, camera_depth, camera_dvs, gnss_sensor, imu_sensor
...
def main():
    ...
    try:
        ...
        vehicle, camera_rgbL, camera_rgbR, camera_semseg, camera_depth, camera_dvs, gnss_sensor, imu_sensor = create_main_actors(
            start_pose, waypoint)
        ...
        with CarlaSyncMode(world, camera_rgbL, camera_rgbR, camera_semseg, camera_depth, camera_dvs, fps=20) as sync_mode:
            ...
            while True:
                ...
                snapshot, image_rgbL, image_rgbR, image_semseg, image_depth, image_dvs = sync_mode.tick(
                    timeout=2.0)
                ...
                # Draw the display.
                draw_image(display, image_dvs, image_rgbL, image_rgbR, image_semseg, image_depth, t_world)

...
try:
    ...
    path1 = os.path.join(parent_dir, "Sync_Calib_mono_i_%s" % (m.name))
    os.mkdir(path1)
...
def draw_image(surface, image_dvs, image_rgb, image_semseg, image_depth, t_world):
    ...
    if self.recording:
        outputfile = 'Sync_Calib_mono_i_%s/davis/frames/%d.png' % (m.name, t_world)
        cv2.imwrite(outputfile, dvs_img)
        image_rgb.save_to_disk('Sync_Calib_mono_i_%s/rgb/frames/%d.png' % (m.name, t_world))
        image_depth.save_to_disk('Sync_Calib_mono_i_%s/depth/frames/%d.png' % (m.name,t_world))
        image_semseg.save_to_disk('Sync_Calib_mono_i_%s/semantic/frames/%d.png' % (m.name, t_world))

...
try:
    ...
    path1 = os.path.join(parent_dir, "Async_stereo_i_%s" % (m.name))
    os.mkdir(path1)
...
def _RGBL_sensor_callback(sensor_data, sensor_name, display, t_world):
    draw_rgbL_image(display, sensor_data)
    if self.recording:
        sensor_data.save_to_disk('Async_stereo_i_%s/rgbL/frames/%d.png' % (m.name, t_world))
        file_rgbL.write("%d;%d.png \n" % (t_world, t_world))
        
def _RGBR_sensor_callback(sensor_data, sensor_name, display, t_world):
    draw_rgbR_image(display, sensor_data)
    if self.recording:
        sensor_data.save_to_disk('Async_stereo_i_%s/rgbR/frames/%d.png' % (m.name, t_world))
...
def _DVS_sensor_callback(sensor_data, sensor_name, display, t_world):
    ...
    if self.recording:
        outputfile = 'Async_stereo_i_%s/davis/frames/%d.png' % (m.name, t_world)
...
def _DEPTH_sensor_callback(sensor_data, sensor_name, display, t_world):
    ...
    if self.recording:
        sensor_data.save_to_disk('Async_stereo_i_%s/depth/frames/%d.png' % (m.name, t_world))
...
def _SSEG_sensor_callback(sensor_data, sensor_name, display, t_world):
    ...
    if self.recording:
        sensor_data.save_to_disk('Async_stereo_i_%s/semantic/frames/%d.png' % (m.name, t_world))

...
try:
    ...
    path1 = os.path.join(parent_dir, "Sync_stereo_i_%s" % (m.name))
    os.mkdir(path1)
...
def draw_image(surface, image_dvs, image_rgbL, image_rgbR, image_semseg, image_depth, t_world):
    ...
    if self.recording:
        outputfile = 'Sync_stereo_i_%s/davis/frames/%d.png' % (m.name, t_world)
        cv2.imwrite(outputfile, dvs_img)
        image_rgbL.save_to_disk('Sync_stereo_i_%s/rgbL/frames/%d.png' % (m.name, t_world))
        image_rgbR.save_to_disk('Sync_stereo_i_%s/rgbR/frames/%d.png' % (m.name, t_world))
        image_depth.save_to_disk('Sync_stereo_i_%s/depth/frames/%d.png' % (m.name,t_world))
        image_semseg.save_to_disk('Sync_stereo_i_%s/semantic/frames/%d.png' % (m.name, t_world))

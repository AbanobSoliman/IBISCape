...
try:
    ...
    path1 = os.path.join(parent_dir, "Full_Odometry_stereo_i_%s" % (m.name))
    os.mkdir(path1)
...
def create_main_actors(start_pose, waypoint):
    ...
    # Modify the basic attributes of the blueprint
    bp_semseg.set_attribute('image_size_x', '1024')
    bp_semseg.set_attribute('image_size_y', '1024')
    ...
    # Modify the basic attributes of the blueprint
    bp_depth.set_attribute('image_size_x', '1024')
    bp_depth.set_attribute('image_size_y', '1024')
...
def _RGBL_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('Full_Odometry_stereo_i_%s/rgbL/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
...
def _RGBR_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('Full_Odometry_stereo_i_%s/rgbR/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
...
def _DVSR_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        cv2.imwrite('Full_Odometry_stereo_i_%s/davisR/frames/%d.png' % (m.name, dvs_events[0]['t']), dvs_img)
        file_davisR.write("%d,%d,%d.png \n" % (dvs_events[0]['t'], dvs_events[-1]['t'], sensor_data.timestamp*(10**9)))
        np.savez_compressed('Full_Odometry_stereo_i_%s/davisR/events/%d' % (m.name, sensor_data.timestamp*(10**9)), t=dvs_events[:]['t'], x=dvs_events[:]['x'], y=dvs_events[:]['y'], p=dvs_events[:]['pol'])
        
def _DVSL_sensor_callback(sensor_data, sensor_name, display):
    dvs_img, dvs_events = draw_dvsL_image(display, sensor_data)
    if self.recording:
        cv2.imwrite('Full_Odometry_stereo_i_%s/davisL/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)), dvs_img)
        file_davisL.write("%d,%d,%d.png \n" % (dvs_events[0]['t'], dvs_events[-1]['t'], sensor_data.timestamp*(10**9)))
        np.savez_compressed('Full_Odometry_stereo_i_%s/davisL/events/%d' % (m.name, sensor_data.timestamp*(10**9)), t=dvs_events[:]['t'], x=dvs_events[:]['x'], y=dvs_events[:]['y'], p=dvs_events[:]['pol'])
    
def _DEPTH_sensor_callback(sensor_data, sensor_name, display):
    sensor_data.convert(cc.LogarithmicDepth)
    if self.recording:
        sensor_data.save_to_disk('Full_Odometry_stereo_i_%s/depth/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))
...
def _SSEG_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('Full_Odometry_stereo_i_%s/semantic/frames/%d.png' % (m.name, sensor_data.timestamp*(10**9)))

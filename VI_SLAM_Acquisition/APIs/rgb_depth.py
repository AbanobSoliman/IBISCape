...
try:
    ...
    path1 = os.path.join(parent_dir, "RGBD_Odometry_i_%s" % (m.name))
    os.mkdir(path1)
...
def _RGB_sensor_callback(sensor_data, ts, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('RGBD_Odometry_i_%s/rgb/frames/%d.png' % (m.name, ts))
...
def _DEPTH_sensor_callback(sensor_data, ts, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('RGBD_Odometry_i_%s/depth/frames/%d.png' % (m.name, ts))

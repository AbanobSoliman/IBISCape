...
try:
    ...
    path1 = os.path.join(parent_dir, "SVI_Odometry_i_%s" % (m.name))
    os.mkdir(path1)
...
def _RGBL_sensor_callback(sensor_data, sensor_name, display):
    ...
    if self.recording:
        sensor_data.save_to_disk('SVI_Odometry_i_%s/rgbL/frames/%d.png' % (m.name, ts))
...
def _RGBR_sensor_callback(sensor_data, sensor_name, display):
    ...
    ts = sensor_data.timestamp*(10**9)
    if self.recording:
        sensor_data.save_to_disk('SVI_Odometry_i_%s/rgbR/frames/%d.png' % (m.name, ts))
